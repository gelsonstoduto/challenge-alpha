...
interface MovieDao {
    ...
    @Query("UPDATE movies SET inMyList = 0 WHERE title = :title")
    suspend fun removeFromMyList(title: String)

    @Query("UPDATE movies SET inMyList = 1 WHERE title = :title")
    suspend fun addToMyList(title: String)

    @Query("SELECT * FROM movies WHERE title = :title")
    fun findMovieByTitle(title: String): Flow<MovieEntity>

    @Query("SELECT * FROM movies WHERE inMyList = 0 and title != :title")
    fun suggestedMovies(title: String): Flow<List<Movie>>
